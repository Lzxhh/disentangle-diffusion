...
from utils1 import to_np_image
...
class SegmentationNetwork():
    def __init__(self, 
                 n_classes = 19,
                 state_dict_path = "pretrained/79999_iter.pth", 
                 device = "cuda"):
    ...
    def get_mask(self, q, label):
        if label == "ear":
            mask_earl = self.get_mask_(q, "l_ear")
            mask_earr = self.get_mask_(q, "r_ear")
            mask_ear = (mask_earl+mask_earr).clamp(0,1) 
            return mask_ear

        elif label == "eye":
            mask_eyel = self.get_mask_(q, "l_eye")
            mask_eyer = self.get_mask_(q, "r_eye")
            mask_eye = (mask_eyel + mask_eyer).clamp(0,1)
            return mask_eye

        elif label == "hair":
            mask_hair = self.get_mask_(q, "hair")
            return mask_hair
        
        elif label == "background":
            mask_hair = self.get_mask_(q, "background")
            return mask_hair
        
        elif label == "mouth":
            mask_mouth_in = self.get_mask_(q, "mouth")
            mask_mouth_u_lip = self.get_mask_(q, "u_lip")
            mask_mouth_l_lip = self.get_mask_(q, "l_lip")
